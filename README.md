...
- поиск по сообщениям, именам и JID;
- экспорт текущего аккаунта/чата в JSON или CSV;
- кнопка `Загрузить старше`, которая пытается запросить более старые сообщения через Baileys `fetchMessageHistory`;
- просмотр скачанных медиафайлов из `MEDIA_DIR`;
- отображение имен контактов, если WhatsApp отдал их через синхронизацию.

Старая история загружается best-effort. WhatsApp Multi-Device может не отдать всю переписку новому связанному устройству. Лучший шанс получить больше истории: `SYNC_FULL_HISTORY=true`, активная сессия и перезапуск Private Core после успешной привязки.
